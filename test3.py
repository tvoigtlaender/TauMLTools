...
for input_i in used_inputs:
    if (input_i.startswith("pfCand_") or input_i.startswith("ele_") or input_i.startswith("muon_")):
        b[input_i]=a[input_i][ak.all(a[input_i]!=-3.4028235e+38,axis=1)]
    else:
        # pass
        b[input_i]=a[input_i][a[input_i]!=-3.4028235e+38]

a_preprocessed = {}
for collection in ["global", "pfCand", "ele", "muon"]:
    a_preprocessed[collection] = {}
...
vertex_z_valid = np.isfinite( a['pfCand_vertex_z'])
...
has_track_details_dxy_finite = has_track_details * np.isfinite(a['pfCand_dxy'])
has_track_details_dxy_sig_finite = has_track_details * np.isfinite(np.abs(a['pfCand_dxy'])/a['pfCand_dxy_error'])
has_track_details_dz_finite = has_track_details * np.isfinite(a['pfCand_dz'])
has_track_details_dz_sig_finite = has_track_details * np.isfinite(np.abs(a['pfCand_dz'])/a['pfCand_dz_error'])
...
muon_dxy_sig_finite = np.isfinite(np.abs(a['muon_dxy'])/a['muon_dxy_error'])
a_preprocessed['muon']['dxy_sig'] = ak.where(muon_dxy_sig_finite, (np.abs(a['muon_dxy'])/a['muon_dxy_error']), 0)

muon_normalizedChi2_valid = ((a['muon_normalizedChi2'] > 0) * np.isfinite(a['muon_normalizedChi2']))
...
muon_pfEcalEnergy_valid = (a['muon_pfEcalEnergy'] >= 0)
