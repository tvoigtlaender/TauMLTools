...
path_template = "root://eosuser.cern.ch///eos/cms/store/group/phys_tau/TauML/prod_2018_v2/ShuffleMergeSpectral_trainingSamples-2_rerun/ShuffleMergeSpectral_{}.root"
...
def num_valid_entries(col):
    if col.ndim == 1:
        return ak.sum(is_finite(col))
    elif col.ndim == 2:
        return ak.sum(ak.flatten(is_finite(col)))
    else:
        raise ValueError(f"Unsupported dimension: {col.ndim}")

def num_entries(col):
    if col.ndim == 1:
        return ak.num(col, axis=0)
    elif col.ndim == 2:
        return ak.num(ak.flatten(col), axis=0)

def convert_to_type(df, new_type):
    return {key: new_type(val) for key, val in df.items()}

all_values = {}

for i in range(0, 500):
    try:
        a = uproot.dask("{}:taus".format(path_template.format(i)),step_size="10 GB", timeout=300)
        print("File {} opened".format(i))
    except Exception as e:
        print("File {} not found".format(i), e)
        continue
...
# Filter non_valid tau candidates with no tau information
    ...
    for input_i in used_inputs:
        # print(get_type(a[input_i]))
        tmp_dict[input_i] = num_valid_entries(a[input_i])
        
    tmp_dict["global_len"] = num_entries(a["tau_pt"])
    tmp_dict["pfCand_len"] = num_entries(a["pfCand_phi"])
    tmp_dict["ele_len"] = num_entries(a["ele_phi"])
    tmp_dict["muon_len"] = num_entries(a["muon_phi"])

    tmp_dict = convert_to_type(dask.compute(tmp_dict)[0], int)
    
    all_values[i] = tmp_dict
    
    # print(tmp_dict)
    with open(f"checks/check{i}.json", "w") as f:
...
with open(f"checks/check_all.json", "w") as f:
    f.write(json.dumps(all_values, indent=4))
print("Done")